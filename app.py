...
- השורה האחרונה (יתרת הכספים בסוף התקופה) חייבת להיות שווה לסכום כל השורות מעליה. השתמש בזה כבדיקת נכונות.
- שורות שליליות (-) חייבות להופיע עם מינוס

הוראות ספציפיות לטבלא ה (פירוט הפקדות):
- הטבלה עשויה להיות ערוכה מימין לשמאל — קרא בזהירות
- סדר העמודות הוא: מועד הפקדה | עבור חודש משכורת | משכורת | תגמולי עובד | תגמולי מעסיק | פיצויים | סה"כ הפקדות
- חלץ כל שורה בנפרד — גם שורות קטנות עם סכומים נמוכים
- הסה"כ בשורת הסיכום חייב להתאים לסכום כל שורות הפקדות
